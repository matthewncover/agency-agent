...
def _person(row) -> Person:
    return Person(
        ...
        morning_prompt_local_time=row.morning_prompt_local_time,
    )
...
class SqlAlchemyProfileRepository(ProfileRepositoryPort):
    ...
    def get_person(self, profile_id: int) -> Person | None:
        with self._engine.connect() as c:
            row = c.execute(
                select(t.person).where(t.person.c.profile_id == profile_id)
            ).one_or_none()
    ...
    def get_kind(self, profile_id: int) -> ProfileKind | None:
        with self._engine.connect() as c:
            k = c.execute(
                select(t.profile.c.kind).where(t.profile.c.id == profile_id)
            ).scalar_one_or_none()
    ...
    def list_group_members(self, group_profile_id: int) -> list[int]:
        with self._engine.connect() as c:
            rows = (
                c.execute(
                    select(t.group_member.c.member_person_id).where(
                        t.group_member.c.group_profile_id == group_profile_id
                    )
                )
                .scalars()
                .all()
    ...
    def list_groups_for_person(self, person_id: int) -> list[int]:
        with self._engine.connect() as c:
            rows = (
                c.execute(
                    select(t.group_member.c.group_profile_id).where(
                        t.group_member.c.member_person_id == person_id
                    )
                )
                .scalars()
                .all()
    ...
    def upsert_profile_doc(self, doc: ProfileDoc) -> ProfileDoc:
        with self._engine.begin() as c:
            existing = c.execute(
                select(t.profile_doc.c.person_id).where(
                    t.profile_doc.c.person_id == doc.person_id
                )
            ).one_or_none()
            if existing:
                c.execute(
                    update(t.profile_doc)
                    .where(t.profile_doc.c.person_id == doc.person_id)
                    .values(content=doc.content)
                )
            else:
                c.execute(
                    insert(t.profile_doc).values(
                        person_id=doc.person_id, content=doc.content
                    )
                )
            row = c.execute(
                select(t.profile_doc).where(t.profile_doc.c.person_id == doc.person_id)
            ).one()
        return ProfileDoc(
    ...
    def get_profile_doc(self, person_id: int) -> ProfileDoc | None:
        with self._engine.connect() as c:
            row = c.execute(
                select(t.profile_doc).where(t.profile_doc.c.person_id == person_id)
            ).one_or_none()
        return (
            ProfileDoc(
                person_id=row.person_id, content=row.content, updated_at=row.updated_at
