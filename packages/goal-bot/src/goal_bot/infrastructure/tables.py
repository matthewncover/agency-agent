...
chapter = Table(
    ...
    Column(
        "owner_profile_id", BigInteger,
        ForeignKey("profile.profile.id", ondelete="RESTRICT"), nullable=False,
    ),
    Column("label", Text),
...
goal_version = Table(
    "goal_version", metadata,
    Column("id", BigInteger, primary_key=True),
    Column(
        "goal_id", BigInteger,
        ForeignKey("goalbot.goal.id", ondelete="CASCADE"), nullable=False,
    ),
    ...
    Column(
        "effective_from", TIMESTAMP(timezone=True),
        server_default=func.now(), nullable=False,
    ),
...
goal_state = Table(
    ...
    Column(
        "goal_id", BigInteger,
        ForeignKey("goalbot.goal.id", ondelete="CASCADE"), primary_key=True,
    ),
...
daily_plan = Table(
    ...
    Column(
        ...
        ForeignKey("profile.person.profile_id", ondelete="CASCADE"), nullable=False,
    ),
...
daily_plan_item = Table(
    "daily_plan_item", metadata,
    Column("id", BigInteger, primary_key=True),
    Column(
        "daily_plan_id", BigInteger,
        ForeignKey("goalbot.daily_plan.id", ondelete="CASCADE"), nullable=False,
    ),
    Column(
        "goal_id", BigInteger,
        ForeignKey("goalbot.goal.id", ondelete="RESTRICT"), nullable=False,
    ),
